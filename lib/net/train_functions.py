...
def model_joint_fn_decorator():
    ...
    def model_fn(model, data, is_eval=False):
        if cfg.RPN.ENABLED:
            ...
            if not cfg.RPN.FIXED:
                ...
                rpn_cls_label = (
                    torch.from_numpy(rpn_cls_label).cuda(non_blocking=True).long()
                )
                rpn_reg_label = (
                    torch.from_numpy(rpn_reg_label).cuda(non_blocking=True).float()
                )
    ...
    def get_rcnn_loss(model, ret_dict, tb_dict):
        ...
        if fg_sum != 0:
            ...
            anchor_size = (
                all_anchor_size[fg_mask] if cfg.RCNN.SIZE_RES_ON_ROI else MEAN_SIZE
            )
