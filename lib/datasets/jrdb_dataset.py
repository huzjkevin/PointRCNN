...
import lib.utils.jrdb_utils as u
...
class JRDBBoxRegressionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # target[0] = target[0] - det_center[-1] # for regressing cz
        target[:3] -= det_center  # for regressing cx, cy, cz
...
if __name__ == "__main__":
    ...
    print("avg input points: ", np.mean(input_len))
    print("max input points: ", np.max(input_len))
    print("min input points: ", np.min(input_len))
    print("num of segment len < 10", len(input_len[input_len <= 10]))
    print("num of segment len < 20", len(input_len[input_len <= 20]))
    print("num of segment len < 50", len(input_len[input_len <= 50]))
    print("num of segment len < 512", len(input_len[input_len <= 512]))
    print("num of segment len < 1024", len(input_len[input_len <= 1024]))
    ...
    ax.hist(input_len, n_bins, histtype="bar", color="blue")
