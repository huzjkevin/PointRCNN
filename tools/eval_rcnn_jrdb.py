...
def eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        # _XY_LIM = (-7, 7)
        # _Z_LIM = (-1, 2)
        # _SAVE_DIR = '../tmp_img_rcnn'
        # os.makedirs(_SAVE_DIR, exist_ok=True)
        # # boxes = pred_boxes3d.data.cpu().numpy()
        ...
        for k in range(batch_size):
            ...
            detections_to_file = []
            for pred_box, score in zip(pred_boxes3d_selected, scores_selected):
                detection_line = f"Pedestrian 0 0 100 0 -1 -1 -1 -1 {pred_box[5]} {pred_box[4]} {pred_box[3]} {pred_box[0]} {pred_box[1]} {pred_box[2]} {pred_box[6]} {score[0]}\n"
            ...
            # ax_bev.set_xlim(_XY_LIM[0], _XY_LIM[1])
            # ax_bev.set_ylim(_XY_LIM[0], _XY_LIM[1])
            # ax_bev.set_title(f"Frame {sample_id[0]}-{k}")
            # ax_bev.set_xlabel("x [m]")
            # ax_bev.set_ylabel("y [m]")
            # # ax_bev.axis("off")
            
            # ax_bev.scatter(pt_xyz[0], pt_xyz[1], s=1, c=_get_pts_color(pt_xyz, 2))
            ...
            #     ax_fpv.set_title(f"Frame {sample_id[0]}-{k}. ")
            #     ax_fpv.set_xlabel("x [m]" if dim == 0 else "y [m]")
            #     ax_fpv.set_ylabel("z [m]")
            #     # ax_fpv.axis("off")

            #     ax_fpv.scatter(pt_xyz[dim], pt_xyz[2], s=1, c=_get_pts_color(pt_xyz, 2))

            # # gt boxes
            # for idx, gt_box in enumerate(data['gt_boxes3d'][k]):
            #     gt_xyz = np.array(gt_box[:3])
            #     gt_lwh = np.array(gt_box[3:-1])
            #     gt_rot_z = gt_box[-1]
            #     gt_box = Box3d(gt_xyz, gt_lwh, gt_rot_z)
            ...
            #     gt_box.draw_bev(ax_bev, c="red")
            #     for dim, ax_fpv in zip((0, 1), (ax_fpv_xz, ax_fpv_yz)):
            #         gt_box.draw_fpv(ax_fpv, dim=dim, c="red")

            # # rpn boxes
            # for idx, rpn_box in enumerate(pred_boxes3d_selected):
            #     rpn_xyz = np.array(rpn_box[:3])
            #     rpn_lwh = np.array(rpn_box[3:-1])
            #     rpn_rot_z = rpn_box[-1]
            #     rpn_box = Box3d(rpn_xyz, rpn_lwh, rpn_rot_z)
            ...
            #     rpn_box.draw_bev(ax_bev, c="blue")
            #     for dim, ax_fpv in zip((0, 1), (ax_fpv_xz, ax_fpv_yz)):
            #         rpn_box.draw_fpv(ax_fpv, dim=dim, c="blue")

            # plt.savefig(os.path.join(_SAVE_DIR, f"frame_{sample_id[0]:04}-{k}.png"))
...
def eval_single_ckpt(root_result_dir):
    ...
    test_loader = create_dataloader_jrdb(logger)
...
def create_dataloader_jrdb(logger):
    ...
    test_loader = DataLoader(test_set, batch_size=args.batch_size, pin_memory=True,
                              num_workers=args.workers, shuffle=False, collate_fn=test_set.collate_batch,
                              drop_last=True)
